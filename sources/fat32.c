...
/*   Updated: 2019/12/17 18:12:37 by llacaze          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			check_for_host_arch(uint32_t nfat_arch, struct fat_arch *fat_arch, t_file file)
{
	...
	while (i < nfat_arch)
	{
		...
		{
			check_file((t_file){file.filename, file.size, (void *)file.ptr + ifswap32(fat_arch->offset, file.reverse), file.reverse, file.bin});
			return (1);
		}
		fat_arch = (void *)fat_arch + sizeof(struct fat_arch);
		i++;
